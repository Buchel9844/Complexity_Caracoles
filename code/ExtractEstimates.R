...
if (sum(colSums(df_inclusion_nat[,grep(pattern = "^n.*\\inclus$", x = colnames(df_inclusion_nat), value = T)],na.rm=T)) > 0) {
 ...
 if(nrow(df_inclusion_nat_relevant) != 0 ){
  ...
  for( i in 1:nrow(df_inclusion_nat_relevant)){
    position.n <- match(str_split_1(df_inclusion_nat_relevant[i,"interactor"],","),
                        str_split_1(df_inclusion_nat_relevant[i,"potential.interactor"],","))
    name.all <- str_split_1(df_inclusion_nat_relevant[i,"potential.interactor"],",")
   ...
   if(df_inclusion_nat_relevant[i,"interaction"] == c("beta_FV_hat_ijf")){
     name.all <-paste0(rep(c(str_split_1(df_inclusion_nat[which(df_inclusion_nat$interaction == "alpha_hat_ij"),
                                                 "potential.interactor"],","),
                    focal),each=DataVec$FV),
              "_",
              str_split_1(df_inclusion_nat[which(df_inclusion_nat$interaction == "gamma_FV_hat_if"),
                                           "potential.interactor"],","))
     position.n <- match(str_split_1(df_inclusion_nat_relevant[i,"interactor"],","),
                         name.all)
     name.all <- str_split_1(df_inclusion_nat_relevant[i,"potential.interactor"],",")
    ...
    if(df_inclusion_nat_relevant[i,"interaction"] == c("beta_H_hat_ijh")){
      name.all <- paste0(rep(c(str_split_1(df_inclusion_nat[which(df_inclusion_nat$interaction == "alpha_hat_ij"),
             ...
             "_",
             str_split_1(df_inclusion_nat[which(df_inclusion_nat$interaction == "gamma_H_hat_ih"),
                                          "potential.interactor"],","))
