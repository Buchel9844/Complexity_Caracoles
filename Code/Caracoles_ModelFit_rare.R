...
FocalLetter <- "L" # "W" or "A"
FocalPrefix <- "LEMA" # "Waitzia" or "ARCA"
FocalSpecies <- "Leontodum marocanus" # "Waitzia.acuminata" or "Arctotheca.calendula"
...
complexitylevel <- levels(as.factor(plant.class$rareORabundant)) # change at line 35 too !!!
...
levels.of.focal <- plant.class[which(plant.class$code.plant==FocalPrefix),"rareORabundant"]
...
year <- as.integer(as.factor(SpDataFocal$year))
plot <- as.integer(as.factor(SpDataFocal$plot))
...
for (i in 1:nyear){
  return(acf(PrelimPosteriors$lambdas[,i,1]))
  return(acf(PrelimPosteriors$lambdas[,i,2]))
}
return(acf(PrelimPosteriors$alpha_generic[,1]))
return(acf(PrelimPosteriors$alpha_intra[,1]))
return(acf(PrelimPosteriors$beta_generic[,1]))
...
Inclusion_ij <- matrix(data = 0, nrow = 3, ncol = S)
...
for(i in 1:nyear){
  ...
  beta_Inclusion_ij$year <- year[i]
  beta_Inclusion <- rbind(beta_Inclusion,beta_Inclusion_ij)
}


write.csv(beta_Inclusion,
          "/home/lisavm/Simulations/beta_Inclusion.csv")
write.csv(Inclusion_ij,
          "/home/lisavm/Simulations/Inclusion_ij.csv")
