...
FullData <- read.csv("data/water_full_env.csv")
...
BetaPlot <- array(NA, dim = c(2,3,EnvLength))
...
beta_SpecificPlot <- array(NA, dim = c(2,2, 3, EnvLength))
...
for(i in 1:EnvLength){
  ...
  # Calculate the posterior distribution for the relevant beta_e,i,j at this particular environmental value
  beta_GenericPost <- exp(Phos$Post$beta_generic[,1] + Phos$Post$beta_generic[,2] * PlotPhos[i])
  beta_Sp1Post <- exp(Phos$Post$beta_generic[,1] + Phos$Post$beta_hat_ij[,2,17] + Phos$Post$beta_generic[,2] * PlotPhos[i])
  beta_Sp2Post <- exp(Phos$Post$beta_generic[,1] + Phos$Post$beta_hat_ij[,1,37] + Phos$Post$beta_generic[,2] * PlotPhos[i])
...
for(i in 1:2){
  lines(x = PlotPhos, y = SpecificPlot[1,i,1,], lwd = 1.5, col = AlphaCols[i])
