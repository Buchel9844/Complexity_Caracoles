...
unlist(strsplit(SpNames_FvH, "_"))

FvH_Fv <- length(complexitylevel.FV[complexitylevel.FV %in% unlist(strsplit(SpNames_FvH, "_"))])
FvH_h <- length(complexitylevel.H[complexitylevel.H %in% unlist(strsplit(SpNames_FvH, "_"))])
...
#load(paste0("results/PrelimFit",FocalPrefix,"_",years,"_",complexity.plant,"_",complexity.animal,".rds"))
...
#save(file=paste0("results/PrelimFit",FocalPrefix,"_",years,"_",complexity.plant,"_",complexity.animal,".rds"),
#     PrelimFit)
...
Inclusion_FV<- matrix(data = 0,nrow = 1, ncol = FV,
...
beta_Inclusion_H<- matrix(data = 0,nrow = length(SpNames), 
                          ...
                          dimnames=list(SpNames,
                                        SpNames_H))
beta_Inclusion_2FV<- matrix(data = 0,nrow = FV,
                            ncol = FV,
                            dimnames=list(c(names(SpTotals_FV[SpToKeep_FV])),
                                          c(names(SpTotals_FV[SpToKeep_FV]))))
beta_Inclusion_2H<- matrix(data = 0,nrow = H, 
                           ncol =H,
                           dimnames=list(SpNames_H,SpNames_H))
beta_Inclusion_FvH<- matrix(data = 0,nrow = FV,
                            ncol = H,
                            dimnames=list(c(names(SpTotals_FV[SpToKeep_FV])),
                                          SpNames_H))
...
for(s in 1:length(SpNames)){
  ...
  if(PrelimPosteriors$alpha_hat_ij[,s])
...
##### Diagnostic plots
pdf(paste0("figure/Prelimfit_",paste(FocalPrefix,years,complexity.plant,complexity.animal,sep="_"),".pdf"))
...
run_estimation = 1
DataVec <- c("N", "S", "H","FV","Intra","run_estimation","RemoveFvH",
             "Fecundity", "SpMatrix",
             "SpMatrix_H","SpMatrix_FV",
             "matrix_HOIs_plant","matrix_HOIs_ijh",
             "matrix_HOIs_ijf", "matrix_HOIs_ihh",
...
save(file=paste0("results/FinalFit",FocalPrefix,"_",years,"_",complexity.plant,"_",complexity.animal,".rds"),
     FinalFit)
load(paste0("results/FinalFit",FocalPrefix,"_",years,"_",complexity.plant,"_",complexity.animal,".rds")) 
...
save(FinalFit, SpNames, N,S,FV,H, Fecundity,years,complexity.plant,RemoveFvH,
     FocalPrefix, 
